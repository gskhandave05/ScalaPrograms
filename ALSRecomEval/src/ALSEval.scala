...
object ALSEval {
  ...
  def getRank(actual: Array[(Int, Double)], predicted: Array[(Int, Double)]): Double = {
    val size = predicted.size
    var sum: Double = 0

    val actRatArray = actual.map(l => l._2)

    val asr = adSum(actRatArray)
    predicted.map(l => sum += ((l._2 * (predicted.indexOf(l) + 1) / size)).toDouble / asr)
    sum
  }
...
//sc.setCheckpointDir("hdfs://bd-hm11:9000/hadoop-user/data/recommendation/checkpoints/")
